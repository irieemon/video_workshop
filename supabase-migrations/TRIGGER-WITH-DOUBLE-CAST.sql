-- Create trigger with double-explicit JSONB casting
...
DECLARE
    test_id UUID;
BEGIN
    SELECT id INTO test_id FROM series_characters LIMIT 1;

    IF test_id IS NOT NULL THEN
        RAISE NOTICE 'Testing trigger on character ID: %', test_id;
        UPDATE series_characters
        SET visual_fingerprint = visual_fingerprint
        WHERE id = test_id;
        RAISE NOTICE '✅ Trigger test successful!';
    ELSE
        RAISE NOTICE 'No characters found to test';
    END IF;
END $$;

-- If test passed, regenerate all templates
UPDATE series_characters
SET visual_fingerprint = visual_fingerprint
...
SELECT '✅ All character templates regenerated!' as status;
